...
Artisan::command('backup:daily {--keep=14}', function () {
    ...
    $mariadbDir = $backupRoot . "/mariadb_{$timestamp}";
    $mariadbArchive = "{$mariadbDir}.tar.gz";
    ...
    if (!in_array($dbConfig['driver'], ['mysql', 'mariadb'])) {
        $this->error('backup:daily currently supports mysql/mariadb connections only.');
        return 1;
    }

    // Resolve mariadb-backup binary (or use env override)
    $backupBinary = env('MARIADB_BACKUP_BINARY') ?: trim(`command -v mariadb-backup`);

    if (!$backupBinary) {
        $this->error('mariadb-backup binary not found. Set MARIADB_BACKUP_BINARY or install mariadb-backup.');
        return 1;
    }
    ...
    // Physical backup via mariadb-backup into target dir
    File::deleteDirectory($mariadbDir);

    $backupCommand = "{$backupBinary} --backup --host={$host} --port={$port} --user={$username} --password={$password} --target-dir=" . escapeshellarg($mariadbDir);
    $process = Process::fromShellCommandline($backupCommand);
    $process->setTimeout(900);
    $process->run(null, array_filter(['MYSQL_PWD' => $password], fn($value) => $value !== null));

    if (!$process->isSuccessful()) {
        $this->error('MariaDB backup failed: ' . $process->getErrorOutput());
        return 1;
    }

    // Archive the physical backup directory for storage/transfer
    $archiveProcess = Process::fromShellCommandline(
        'tar -czf ' . escapeshellarg($mariadbArchive) . ' -C ' . escapeshellarg($mariadbDir) . ' .'
    );
    $archiveProcess->setTimeout(600);
    $archiveProcess->run();

    if (!$archiveProcess->isSuccessful()) {
        $this->error('Failed to archive MariaDB backup: ' . $archiveProcess->getErrorOutput());
        return 1;
    }

    // Clean up extracted and temp backup dirs left by mariadb-backup
    File::deleteDirectory($mariadbDir);
    File::deleteDirectory($mariadbDir . '_tmp');
    ...
    $this->info("MariaDB backup saved to: {$mariadbArchive}");
    ...
    return 0;
})->purpose('Back up the database and archive the documents directory.');
