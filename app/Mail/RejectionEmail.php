...
/**
 * Mailable notifying the event creator that their event was rejected.
 *
 * This class renders the `mail.rejection-email` Blade view and is safe to
 ...
 * time, location, and the reason for the rejection).
 *
 * Typical usage:
 *  Mail::to($creatorEmail)->send(new RejectionEmail($eventData, $justification));
 *  // or queue(): Mail::to($creatorEmail)->queue(new RejectionEmail($eventData, $justification));
 *
 * @package App\Mail
 */
class RejectionEmail extends Mailable implements ShouldQueue
{
    ...
    public function envelope(): Envelope
    {
        ...
        return new Envelope(
            subject: "Event Rejected: {$subjectTitle}",
        );
    }
