...
# Backup restore configuration (optional)
BACKUP_DIR="/home/${APP_SYSTEM_USER}/"
DB_BACKUP_FILE="${BACKUP_DIR}/dbdump.sql.gz"
DOCS_BACKUP_ZIP="${BACKUP_DIR}/documents.zip"
...
echo "Step 8: Installing MariaDB..."
echo "-----------------------------------"
...
touch /etc/nginx/sites-available/eventflow.conf
# Create eventflow server block
cat > /etc/nginx/sites-available/eventflow.conf<< 'EOF'
server {
    listen 80;
...
ln -s /etc/nginx/sites-available/eventflow.conf /etc/nginx/sites-enabled/eventflow.conf
