...
it('deletes use requirements for a given venue ID', function () {
    ...
    $result = $this->useRequirementService->deleteUseRequirement($venue->id);
...
it('returns false when no use requirements exist for given venue ID', function () {
    $result = $this->useRequirementService->deleteUseRequirement(999);

    expect($result)->toBeFalse();
});

it('throws InvalidArgumentException when venue ID is negative in deleteUseRequirement', function () {
    $this->expectException(InvalidArgumentException::class);
    $this->expectExceptionMessage('UseRequirement ID must be a positive integer.');

    $this->useRequirementService->deleteUseRequirement(-10);
});
