...
@section('subject', 'Request Submitted')
@section('preheader', 'We received your event request.')

@section('content')
    <h3>Your event request was submitted</h3>
    <p style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:15px;line-height:22px;color:#212529;margin:0 0 12px;">
        We received your request and will review it soon. Here are the details you submitted:
    </p>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border:1px solid #e6e6e6;border-radius:8px;overflow:hidden;">
                    <tr>
                        <td style="padding:16px 20px;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:16px;line-height:24px;color:#212529;border-bottom:1px solid #e6e6e6;mso-line-height-rule:exactly;">
                            ...
                            <div>Advisor: {{ $event['organization_advisor_name'] ?? '—' }} ({{ $event['organization_advisor_email'] ?? 'N/A' }})</div>
